...
void main() {
  ...
  test('exports and atomically merges authenticated recovery records', () async {
    ...
    expect(destination.recoveryReceipts.single.transferId, receipt.transferId);
    expect(
      await destination.mergeRecoveryRecords(
        canonicalRecords: records,
        receipt: receipt,
    ...
    await expectLater(
      destination.mergeRecoveryRecords(
        ...
        authorizeSender: (_) async =>
            throw StateError('Conflicting replay must fail first.'),
      ),
      throwsA(isA<FormatException>()),
    );
    expect(destination.events, hasLength(2));
    expect(destination.recoveryReceipts, hasLength(1));
  ...
  test('rejects conflicting overlap without a partial merge', () async {
    final root = await Directory.systemTemp.createTemp(
      'yappa-mls-recovery-overlap-',
    );
    final secrets = _MemorySecrets();
    addTearDown(() => root.delete(recursive: true));
    final source = await MlsEventStore.open(
    ...
    await source.apply(
      _historyEvent(sequence: 1, eventId: 's' * 22),
      senderIsOwner: false,
    );
    final records = source.exportRecoveryRecords(
      firstServerSequence: 1,
    ...
    await destination.apply(
      _historyEvent(sequence: 1, eventId: 'd' * 22),
      senderIsOwner: false,
    );
    await expectLater(
      destination.mergeRecoveryRecords(
    ...
    expect(destination.events.single.eventId, 'd' * 22);
