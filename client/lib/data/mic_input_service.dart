...
class MicInputService extends ChangeNotifier {
  ...
  Future<void> startCapture() async {
    ...
    try {
      ...
      await _safeCancelAudioStreamSubscription();
      ...
      await _safeCancelAmplitudeSubscription();
  ...
  Future<void> _safeCancelAmplitudeSubscription() async {
    ...
    try {
      ...
      if (!(e.code == 'error' &&
          (e.message?.contains('No active stream to cancel') ?? false))) {
        rethrow;
      }
    } catch (_) {}
  }

  Future<void> _safeCancelAudioStreamSubscription() async {
  ...
  double _processLevel(double rawLevel) {
    final gained =
        (rawLevel * YappaAudioPreferences.inputGain).clamp(0.0, 1.0);

    if (YappaAudioPreferences.automaticSensitivity) {
      return gained;
    }
    ...
    final scaled = (gained - threshold) / (1 - threshold);
