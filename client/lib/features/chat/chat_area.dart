...
class _ChatAreaState extends State<ChatArea> {
  ...
  void _syncFocusedParticipant() {
    ...
    final sharingMember = widget.voiceMembers.cast<Member?>().firstWhere(
          (member) => member?.voiceState.screenShareEnabled == true,
          orElse: () => null,
        );
          ...
          child: Column(
            children: [
              if (_isVoiceDeck)
                ...
                Expanded(child: MessageList(messages: widget.messages)),
                MessageInput(
                  key: _messageInputKey,
                  channel: widget.channel,
                  onSend: widget.onSend,
                  onSendWithAttachments: widget.onSendWithAttachments,
                  ...
                  child: Padding(
                    ...
                    child: focusedMember == null
                        ...
                        : _VoiceStageTile(
                            ...
                            isFocused: true,
                            isLocalUser: isInSelectedVoiceDeck &&
                                focusedMember!.voiceChannelId == channel.id,
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                'This area becomes the live call stage. Later, camera feeds and screen shares will replace these placeholders like a Discord call layout.',
                textAlign: TextAlign.center,
                style: TextStyle(
...
class _VoiceStageTile extends StatelessWidget {
  ...
  bool get _cameraActive =>
      isLocalUser ? localCameraEnabled : member.voiceState.cameraEnabled;
...
class _StageStatusPill extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    const color = Color(0xFFFFD37A);

    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 9),
      decoration: BoxDecoration(
        color: highlight ? const Color(0xFF2C2214) : NewChatColors.surface,
        borderRadius: BorderRadius.circular(999),
        border: Border.all(
          color:
              highlight ? color.withValues(alpha: 0.55) : NewChatColors.outline,
        ),
      ),
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          Icon(
            icon,
            size: 15,
            color: highlight ? color : NewChatColors.textMuted,
          ),
